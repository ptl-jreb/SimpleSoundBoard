...
module.exports = {
  ...
  async execute (message, args) {
    ...
    if (message.member.voice.channel) {
      ...
      if (!args[0]) message.reply('You need to provide a category code!')

      let category = categories[args[0]]
      if (!category) {
        message.reply('Category does not exists !')
      } else if (category && !category.length) {
        message.reply('Category does not have sounds !')
      } else {
        ...
        if (!fs.existsSync(path.join(__dirname, '..', 'sounds', sound + '.mp3'))) {
          ...
          if (sound === 'rickrolled') message.reply('You\'ve been RICKROLLED !')
          dispatcher.on('start', () => {
